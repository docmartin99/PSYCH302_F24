...
__QUESTIONS__: For our data, what do you think will be our response variable? Happiness

What other factors do you think might impact the response variable? Fear, Healthy
...
unique(happiness$Education)

unique(happiness$Dominant_hand)
...
alcohol_use <- happiness %>%
  filter(Happiness_score <= 15, 
         Happiness_score >= 0,
         Alcohol %in% c("drink a lot", "never")) %>%
...
steve <- happiness %>%
  ...
  select(Happiness_score, Religiosity_score, Lying, Age)


boxplot(steve$Religiosity_score ~ steve$Lying)

boxplot(steve$Age~steve$Lying)
