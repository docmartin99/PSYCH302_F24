...
The Normal Probability Applet is nice for visualizing areas under the curve.  However, it has significant limitations.  We can use the pnorm(x, $\mu$, $\sigma$) function in R to calculate areas under the curve.  The `p` in `pnorm()` stands for probability and `norm` obviously stands for the normal distribution.  

By default, the `pnorm()` gives the area of the curve to the *LEFT* of the value, $x$, for a normal distribution with a mean, $\mu$, and standard deviation, $\sigma$.  

We can calculate probabilities directly in the original units of the data, or use the z-score with a mean of zero and a standard deviation of 1:  `pnorm(z)`.  
...
We will show you how to calculate these probabilities in R with the examples below.  
...
__NOTE__:  Value 1 is the lower of the two values and Value 2 is the higher value.  


### Calculating Percentiles using a Normal Distribution

A percentile is a number such that a specified percentage of the population are at or below this number. For example, the 25<sup>th</sup> percentile is the number in a data set that is greater than or equal to 25% of all the values in the data set.  We can find the values of percentiles for a normal distribution R using the `qnorm(percentile, mu, sigma)` function.
...
For more detailed instructions revisit [Summarizing Data](../docs/Textbook/Summarizing_Data_Multiple_Groups.html)
...
library(rio)
library(mosaic)
bat_avg <- import("../Data/BattingAverages.xlsx")
```
...
library(rio)
library(mosaic)
body_temp <- import("../Data/BodyTemp.xlsx")
...
4. To calculate probabilities for an observation $x$, calculate the $z$-score using $\mu$, $\sigma$, and $x$ and then use `pnorm(x, mu, sigma)` or `1-pnorm(x, mu, sigma)` to get the desired probability (above, below, between). In every case, the **probability is** given by the **Area** under the curve.  
