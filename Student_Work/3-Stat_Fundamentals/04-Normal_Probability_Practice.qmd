...
The code below can serve as a template for any calculations involving the normal distribution.  Feel free to copy and paste this into the code chunks 

__NOTE__:  R saves any objects that are named.  This means that if you run `mu <- 12`, R keeps track of x until you change it.  When doing multiple problems that all use `mu`, be sure to run the whole code chunk, which will overwrite the names from previous examples.  
