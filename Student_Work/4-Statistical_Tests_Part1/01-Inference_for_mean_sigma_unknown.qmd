...
Because $t$ is not normally distributed, we can't use `pnorm()` to calculate probabilities.  However, R is fully capable of calculating probabilities for many different distributions including the t-distribution.  We have to tell R the t-value *and* the degrees of freedom.
