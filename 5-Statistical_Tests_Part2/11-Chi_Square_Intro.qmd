...
```
<a href="https://raw.githubusercontent.com/byuistats/Math221D_Course/main/Student_Work/Workbooks/Chi_Square_Intro.qmd" download="Chi_Square_Intro.qmd" type="text/markdown" download class="button-download">
    Download QMD
</a>
...
Similarly, when we want to compare a *categorical* variable across multiple groups, we must modify the hypothesis test from the 2-sample proportion z-test and introduce a new test statistic: $\chi^2$.  The Greek letter, $\chi$, is pronounced like "ki" in "kite", __not__ like "chi" in "tai chi".  
...
Categorical data analysis and visualization is usually easier to work with raw data.  We can input `table()` into `chisq.test()` and use `ggplot()` directly.  
