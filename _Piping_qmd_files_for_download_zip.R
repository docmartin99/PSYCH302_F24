...
add_yaml_features_and_remove_button <- function(file_path, new_features) {
  ...
  existing_yaml <- yaml::yaml.load(paste(content[(yaml_start + 1):(yaml_end - 1)], collapse = "\n"))
...
for(i in 1:length(source_dirs)){
  ...
  dest_dir <- path(temp_dir, "Student_Work", str_remove_all(source_dirs[i], "./"))
...
# Move zip file to desired location (e.g., current working directory)
file_move(zip_file_path, path(getwd(), zip_file_name))

cat("Moved zip file to:", path(getwd(), zip_file_name), "\n")
