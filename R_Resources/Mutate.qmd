...
```
<a href="https://raw.githubusercontent.com/byuistats/Math221D_Course/main/Student_Work/R_Resources/Mutate.qmd" download="Mutate.qmd" type="text/markdown" download class="button-download">
    Download QMD
</a>
...
Notice that the mean is `Inf` which means infinity, or undefined.  This is likely due to R attempting to divide a number by zero, meaning someone answered that their arm span was zero.  This is where [`filter()`](../R_Resources/Filter.qmd) comes in handy.  We can filter out the rows where Armspan_cm is 0:
...
1. Click to see how to [filter](../R_Resources/Filter.qmd) outliers of ht_to_wt ratio and [select](../R_Resources/Select.qmd) only the columns of interest.
