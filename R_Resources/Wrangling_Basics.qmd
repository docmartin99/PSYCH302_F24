...
We would begin with the raw data, "pipe" it into the `filter()` function and tell R what I want it to do.  I can either tell are what I want to keep or what I want to discard (more [here](./R_Resources/Filter.qmd##Logical-Operators)):

```
marital_Status_data %>% filter(marital_status != "It's complicated")
```
__NOTE__:  The `!=` is read "not equal to", and is a common [logical operator]() used in computer programming.  So the above code returns a subset of the original data omitting the rows.
